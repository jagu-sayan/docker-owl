...
impl Acme {
    ...
    pub fn new(debug: bool) -> Box<Acme> {
    ...
    pub fn create_account(&self, mail: &str) -> Account {

        // 1. Create AccountRegistration object helper
        let directory = Directory::from_url(self.acme_url).unwrap();
        ...
        let pkey_path_str = pkey_path.to_str().unwrap();
        if pkey_path.is_file() {
            account_registration = account_registration.pkey_from_file(pkey_path_str).unwrap();
        } else {
            println!("PKEY '{}' not found, we will generate one", pkey_path_str);
        }
        ...
        if !pkey_path.is_file() {
            account.save_private_key(pkey_path_str).unwrap();
        }

        account
    }

    fn dns_validation(account: &Account, hostname: &str) {

        // 1. Create Authorization object helper
        let authorization = account.authorization(hostname).unwrap();
    ...
    fn http_validation(account: &Account, hostname: &str, dest_folder: &str) {
        ...
        http_challenge.save_key_authorization(dest_folder).unwrap();
    ...
    fn gen_key(key_size: u32) -> PKey {
        let rsa = Rsa::generate(key_size).unwrap();
        PKey::from_rsa(rsa).unwrap()
    }

    fn sign_certificate(account: &Account, domains: &[&str], keysize: u32) -> SignedCertificate {
...
impl OwlPlugin for Acme {
    ...
    fn should_process(&self, details: &DockerContainerDetails) -> bool {
        details.Config.Labels.contains_key("letsencrypt-enable")
    }
    ...
    fn process(&self, details: &DockerContainerDetails) {
        ...
        match challenge.as_str() {
            ...
            "http" => Acme::http_validation(&account, hostname, dest),
            _ => println!("No challenge found")
        }
        ...
        certificate.save_signed_certificate(format!("{}/{}.pem", dest, domains[0])).unwrap();
        certificate.save_private_key(format!("{}/{}.key", dest, domains[0])).unwrap();
        ...
        if dh_group.is_some() && !dh_path.is_file() {
            ...
            let mut buffer = File::create(dh_path).unwrap();
            buffer.write_all(dh_buffer).unwrap();
        }
    }
}